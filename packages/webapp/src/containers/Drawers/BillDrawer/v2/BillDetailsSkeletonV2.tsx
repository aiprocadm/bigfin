...
/**
 * Скелетон загрузки деталей счёта поставщика: контур шапки + вкладки
 * + карточки. DrawerTitle с sr-only текстом сохраняет доступное имя
 * диалога, пока данные счёта ещё грузятся.
 */
export function BillDetailsSkeletonV2() {
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <DrawerHeader className="shrink-0 gap-2 border-b border-border bg-surface px-4 py-4 pr-14 sm:px-6">
        <DrawerTitle className="sr-only">
          {intl.get('bill.drawer.loading')}
        </DrawerTitle>
