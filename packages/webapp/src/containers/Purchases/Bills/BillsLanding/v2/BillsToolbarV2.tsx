...
const DEFAULT_FIELD_KEY = 'bill_number';
    ...
    <Popover open={open} onOpenChange={handleOpenChange}>
      ...
      <PopoverContent align="start" className="w-80">
        ...
        >
          ...
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="bills-filter-value">
              {intl.get('filter.value')}
            </Label>
            <Input
              id="bills-filter-value"
              value={value}
...
interface BillsToolbarV2RootProps {
  // #withBills
  billsSelectedRows?: number[];
  billsFilterConditions?: FilterRole[];
  billsViewSlug?: string | null;

  // #withBillsActions
  setBillsTableState: (state: Record<string, unknown>) => void;
...
/**
 * Панель действий списка счетов поставщиков (shadcn, замена BillsActionsBar).
 */
function BillsToolbarV2Root({
  billsSelectedRows = [],
  billsFilterConditions = [],
  billsViewSlug,
  setBillsTableState,
  openDialog,
}: BillsToolbarV2RootProps) {
  const history = useHistory();

  // Контекст списка (легаси-провайдер без типов) — кастуем локально.
  const { billsViews = [], fields = [] } = (useBillsListContext() ?? {}) as {
    billsViews?: ResourceView[];
    fields?: ResourceField[];
  };

  const { refresh } = useRefreshBills() as { refresh: () => void };
  ...
  const { openBulkDeleteDialog, isValidatingBulkDeleteBills } =
    useBulkDeleteBillsDialog() as {
      openBulkDeleteDialog: (ids: number[]) => Promise<void>;
      isValidatingBulkDeleteBills: boolean;
    };
  ...
  if (billsSelectedRows.length > 0) {
    return (
      <div className="bigfin-ui">
        <div className="flex items-center gap-3 border-b border-border bg-surface px-4 py-2">
          <span className="text-sm font-medium text-text-secondary tabular-nums">
            {billsSelectedRows.length}
          </span>
          <Button
            variant="ghost"
            size="sm"
  ...
  return (
    <div className="bigfin-ui">
      <div className="flex flex-wrap items-center gap-2 border-b border-border bg-surface px-4 py-2">
        <Select
          value={billsViewSlug ?? ALL_VIEWS_VALUE}
        ...
        <Can I={BillAction.Create} a={AbilitySubject.Bill}>
          <Button size="sm" className="gap-1.5" onClick={handleNewBillBtnClick}>
        ...
        <BillsFilterPopover
          fields={fields}
          conditions={billsFilterConditions}
...
export const BillsToolbarV2 = compose(
  withBillsActions,
  withBills(
    ({
      billsSelectedRows,
      ...
      billsSelectedRows,
      billsFilterConditions: billsTableState.filterRoles ?? [],
      billsViewSlug: billsTableState.viewSlug ?? null,
    }),
  ),
  withDialogActions,
)(BillsToolbarV2Root) as ComponentType<Record<string, never>>;
