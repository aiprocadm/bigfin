...
export const mapEntriesToRuVatLines = (entries: any[]): RuVatMapped => {
  ...
  const lines = (entries || []).map((entry: any, index: number) => {
    ...
    return {
      ...
      priceExclVatText: formatMoneyRu(priceExclVat),
      amountExclVat,
      amountExclVatText: formatMoneyRu(amountExclVat),
      vatRateText: hasVat ? `${Number(entry.taxRate) || 0}%` : NO_VAT,
      vatAmount,
      vatAmountText: hasVat ? formatMoneyRu(vatAmount) : NO_VAT,
      amountInclVat,
      amountInclVatText: formatMoneyRu(amountInclVat),
    };
  });
