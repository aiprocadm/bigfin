...
const DEFAULT_FIELD_KEY = 'display_name';
 ...
 */
function CustomersFilterPopover({
  fields,
  conditions,
  onFilterChange,
}: {
    ...
    <Popover open={open} onOpenChange={handleOpenChange}>
      ...
      <PopoverContent align="start" className="w-80">
        ...
        >
          ...
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="customers-filter-comparator">
              {intl.get('toolbar.filter.condition')}
            </Label>
            <Select value={comparator} onValueChange={setComparator}>
              <SelectTrigger id="customers-filter-comparator">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="customers-filter-value">
...
interface CustomersToolbarV2RootProps {
  // #withCustomers
  customersSelectedRows?: number[];
  customersFilterConditions?: FilterRole[];
  customersInactiveMode?: boolean;
  customersViewSlug?: string | null;

  // #withCustomersActions
  setCustomersTableState: (state: Record<string, unknown>) => void;
...
/**
 * Панель действий списка клиентов (shadcn, замена CustomersActionsBar).
 */
function CustomersToolbarV2Root({
  customersSelectedRows = [],
  customersFilterConditions = [],
  customersInactiveMode = false,
  customersViewSlug,
  setCustomersTableState,
  openDialog,
}: CustomersToolbarV2RootProps) {
  const history = useHistory();

  // Контекст списка (легаси-провайдер без типов) — кастуем локально.
  const { customersViews = [], fields = [] } = (useCustomersListContext() ??
    {}) as {
    customersViews?: ResourceView[];
    fields?: ResourceField[];
  };

  const { refresh } = useRefreshCustomers() as { refresh: () => void };
  ...
  const { openBulkDeleteDialog, isValidatingBulkDelete } =
    useBulkDeleteCustomersDialog() as {
  ...
  const handleNewCustomerBtnClick = () => {
  ...
  if (customersSelectedRows.length > 0) {
    return (
      <div className="bigfin-ui">
        <div className="flex items-center gap-3 border-b border-border bg-surface px-4 py-2">
          <span className="text-sm font-medium text-text-secondary tabular-nums">
            {customersSelectedRows.length}
          </span>
          <Button
            variant="ghost"
            size="sm"
  ...
  return (
    <div className="bigfin-ui">
      <div className="flex flex-wrap items-center gap-2 border-b border-border bg-surface px-4 py-2">
        <Select
          value={customersViewSlug ?? ALL_VIEWS_VALUE}
        ...
        <Can I={CustomerAction.Create} a={AbilitySubject.Customer}>
          <Button size="sm" className="gap-1.5" onClick={handleNewCustomerBtnClick}>
        ...
        <div className="ml-auto flex items-center gap-3">
          <Can I={CustomerAction.Edit} a={AbilitySubject.Customer}>
            <div className="flex items-center gap-2">
              <Switch
                id="customers-inactive-switch"
                checked={customersInactiveMode}
                onCheckedChange={handleInactiveSwitchChange}
              />
              <Label
                htmlFor="customers-inactive-switch"
...
export const CustomersToolbarV2 = compose(
  withCustomersActions,
  withCustomers(
    ({
      customersSelectedRows,
      ...
      customersViewSlug: customersTableState.viewSlug ?? null,
    }),
  ),
  withDialogActions,
)(CustomersToolbarV2Root) as ComponentType<Record<string, never>>;
