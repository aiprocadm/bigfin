...
const DEFAULT_FIELD_KEY = 'payment_receive_no';
 ...
 */
function PaymentsReceivedFilterPopover({
  fields,
  conditions,
  onFilterChange,
}: {
      ...
      <PopoverContent align="start" className="w-80">
        ...
        >
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="payments-received-filter-field">
              {intl.get('toolbar.filter.field')}
            </Label>
            <Select value={fieldKey} onValueChange={setFieldKey}>
              <SelectTrigger id="payments-received-filter-field">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {fields.map((field) => (
          ...
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="payments-received-filter-comparator">
              {intl.get('toolbar.filter.condition')}
            </Label>
            <Select value={comparator} onValueChange={setComparator}>
              <SelectTrigger id="payments-received-filter-comparator">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="payments-received-filter-value">
              {intl.get('filter.value')}
            </Label>
            <Input
              id="payments-received-filter-value"
              value={value}
...
interface PaymentsReceivedToolbarV2RootProps {
  // #withPaymentsReceived
  paymentReceivesSelectedRows?: number[];
  paymentFilterConditions?: FilterRole[];
  paymentReceivesViewSlug?: string | null;

  // #withPaymentsReceivedActions
  setPaymentReceivesTableState: (state: Record<string, unknown>) => void;
...
/**
 * Панель действий списка входящих платежей (shadcn, замена
 * PaymentsReceivedActionsBar).
 */
function PaymentsReceivedToolbarV2Root({
  paymentReceivesSelectedRows = [],
  paymentFilterConditions = [],
  paymentReceivesViewSlug,
  setPaymentReceivesTableState,
  openDialog,
  openDrawer,
}: PaymentsReceivedToolbarV2RootProps) {
  const history = useHistory();

  // Контекст списка (легаси-провайдер без типов) — кастуем локально.
  const { paymentReceivesViews = [], fields = [] } =
    (usePaymentsReceivedListContext() ?? {}) as {
      paymentReceivesViews?: ResourceView[];
      fields?: ResourceField[];
    };

  const { refresh } = useRefreshPaymentReceive() as { refresh: () => void };
  ...
  const { openBulkDeleteDialog, isValidatingBulkDeletePaymentReceives } =
    useBulkDeletePaymentReceivesDialog() as {
      openBulkDeleteDialog: (ids: number[]) => Promise<void>;
      isValidatingBulkDeletePaymentReceives: boolean;
    };
  ...
  const handleNewPaymentBtnClick = () => {
  ...
  if (paymentReceivesSelectedRows.length > 0) {
    return (
      <div className="bigfin-ui">
        <div className="flex items-center gap-3 border-b border-border bg-surface px-4 py-2">
          <span className="text-sm font-medium text-text-secondary tabular-nums">
            {paymentReceivesSelectedRows.length}
          </span>
          <Button
            variant="ghost"
            size="sm"
  ...
  return (
    <div className="bigfin-ui">
      <div className="flex flex-wrap items-center gap-2 border-b border-border bg-surface px-4 py-2">
        <Select
          value={paymentReceivesViewSlug ?? ALL_VIEWS_VALUE}
        ...
        <Can I={PaymentReceiveAction.Create} a={AbilitySubject.PaymentReceive}>
          <Button size="sm" className="gap-1.5" onClick={handleNewPaymentBtnClick}>
            <Plus className="h-4 w-4" aria-hidden />
            {intl.get('new_payment_received')}
          </Button>
        </Can>

        <PaymentsReceivedFilterPopover
          fields={fields}
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="start">
            ...
            <DropdownMenuItem onClick={handleCustomizeBtnClick}>
              ...
              {intl.get('payment_received.action.customize_templates')}
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
...
export const PaymentsReceivedToolbarV2 = compose(
  withPaymentsReceivedActions,
  withPaymentsReceived(
    ({
      paymentReceivesSelectedRows,
      ...
      paymentReceivesViewSlug: paymentReceivesTableState.viewSlug ?? null,
    }),
  ),
  withDialogActions,
  withDrawerActions,
)(PaymentsReceivedToolbarV2Root) as ComponentType<Record<string, never>>;
