 ...
 * v89 (переработка интерфейса): удалён `Dashboard/TopbarUser.tsx` — меню
 * профиля СТАРОЙ оболочки. Его место занял аватар в `ConnectedTopbar`, а сам
 * файл оставался живым только потому, что его рисовала шапка настроек. Из-за
 * этого, войдя в настройки, человек видел ДВА меню профиля одно под другим.
 * Убрали дубль — файл стал недостижим; порог не менялся (86 → 85).
 * v85: 87 → 85. Удалены два предупреждения оплат поставщикам с пустыми
 * обработчиками подтверждения — их не ставил никто. Вместе с недостижимым
 * ящиком разбора операции ушла и копия поля ветки `BranchSuggestField_`,
 * которую ввозил только он.
 */
const ORPHANS_CEILING = 85;
