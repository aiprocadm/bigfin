...
const ReceiptDetailContentV2 = lazy(() =>
  import('./v2/ReceiptDetailContentV2').then((module) => ({
    default: module.ReceiptDetailContentV2,
  })),
);
...
/**
 * Drawer «Детали чека» (shadcn Drawer + вкладки на v2-контенте).
 * Механизм открытия прежний: redux openDrawer(DRAWERS.RECEIPT_DETAILS,
 * { receiptId }).
 */
function ReceiptDetailDrawerRoot({
  name,
  isOpen,
  payload,
  closeDrawer,
}: ReceiptDetailDrawerProps & WithDrawerActionsProps) {
  ...
  return (
    <Drawer open={Boolean(isOpen)} onOpenChange={handleOpenChange}>
      ...
      >
        ...
        >
          <ReceiptDetailContentV2 receiptId={payload?.receiptId} />
        </Suspense>
      </DrawerContent>
    </Drawer>
  );
...
export default compose(
  withDrawersLoose(),
  withDrawerActions,
)(ReceiptDetailDrawerRoot) as ComponentType<{ name: string }>;
