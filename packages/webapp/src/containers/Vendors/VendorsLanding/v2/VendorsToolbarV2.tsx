    ...
    <Popover open={open} onOpenChange={handleOpenChange}>
      ...
      <PopoverContent align="start" className="w-80">
        ...
        >
          ...
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="vendors-filter-value">
              {intl.get('filter.value')}
            </Label>
            <Input
              id="vendors-filter-value"
              value={value}
...
interface VendorsToolbarV2RootProps {
  // #withVendors
  vendorsSelectedRows?: number[];
  vendorsFilterConditions?: FilterRole[];
  vendorsInactiveMode?: boolean;
  vendorsViewSlug?: string | null;

  // #withVendorsActions
  setVendorsTableState: (state: Record<string, unknown>) => void;
...
/**
 * Панель действий списка поставщиков (shadcn, замена VendorActionsBar).
 */
function VendorsToolbarV2Root({
  vendorsSelectedRows = [],
  vendorsFilterConditions = [],
  vendorsInactiveMode = false,
  vendorsViewSlug,
  setVendorsTableState,
  openDialog,
}: VendorsToolbarV2RootProps) {
  const history = useHistory();

  // Контекст списка (легаси-провайдер без типов) — кастуем локально.
  const { vendorsViews = [], fields = [] } = (useVendorsListContext() ??
    {}) as {
    vendorsViews?: ResourceView[];
    fields?: ResourceField[];
  };

  const { refresh } = useRefreshVendors() as { refresh: () => void };
  ...
  if (vendorsSelectedRows.length > 0) {
    return (
      <div className="bigfin-ui">
        <div className="flex items-center gap-3 border-b border-border bg-surface px-4 py-2">
          <span className="text-sm font-medium text-text-secondary tabular-nums">
            {vendorsSelectedRows.length}
          </span>
          <Button
            variant="ghost"
            size="sm"
  ...
  return (
    <div className="bigfin-ui">
      <div className="flex flex-wrap items-center gap-2 border-b border-border bg-surface px-4 py-2">
        <Select
          value={vendorsViewSlug ?? ALL_VIEWS_VALUE}
        ...
        <Can I={VendorAction.Create} a={AbilitySubject.Vendor}>
          <Button size="sm" className="gap-1.5" onClick={handleNewVendorBtnClick}>
        ...
        <div className="ml-auto flex items-center gap-3">
          <Can I={VendorAction.Edit} a={AbilitySubject.Vendor}>
            <div className="flex items-center gap-2">
              <Switch
                id="vendors-inactive-switch"
                checked={vendorsInactiveMode}
                onCheckedChange={handleInactiveSwitchChange}
              />
              <Label
                htmlFor="vendors-inactive-switch"
...
export const VendorsToolbarV2 = compose(
  withVendorsActions,
  withVendors(
    ({
      vendorsSelectedRows,
      ...
      vendorsViewSlug: vendorsTableState.viewSlug ?? null,
    }),
  ),
  withDialogActions,
)(VendorsToolbarV2Root) as ComponentType<Record<string, never>>;
