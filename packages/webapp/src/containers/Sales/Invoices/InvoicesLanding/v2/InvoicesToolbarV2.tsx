...
const DEFAULT_FIELD_KEY = 'invoice_no';
 ...
 */
function InvoicesFilterPopover({
  fields,
  conditions,
  onFilterChange,
}: {
    ...
    <Popover open={open} onOpenChange={handleOpenChange}>
      ...
      <PopoverContent align="start" className="w-80">
        ...
        >
          ...
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="invoices-filter-value">
...
interface InvoicesToolbarV2RootProps {
  // #withInvoices
  invoicesSelectedRows?: number[];
  invoicesFilterConditions?: FilterRole[];
  invoicesViewSlug?: string | null;

  // #withInvoiceActions
  setInvoicesTableState: (state: Record<string, unknown>) => void;
...
/**
 * Панель действий списка счетов покупателям (shadcn, замена InvoicesActionsBar).
 */
function InvoicesToolbarV2Root({
  invoicesSelectedRows = [],
  invoicesFilterConditions = [],
  invoicesViewSlug,
  setInvoicesTableState,
  openDialog,
  openDrawer,
}: InvoicesToolbarV2RootProps) {
  const history = useHistory();

  // Контекст списка (легаси-провайдер без типов) — кастуем локально.
  const { invoicesViews = [], invoicesFields = [] } =
    (useInvoicesListContext() ?? {}) as {
      invoicesViews?: ResourceView[];
      invoicesFields?: ResourceField[];
    };

  const { refresh } = useRefreshInvoices() as { refresh: () => void };
  ...
  const { openBulkDeleteDialog, isValidatingBulkDeleteInvoices } =
    useBulkDeleteInvoicesDialog() as {
      openBulkDeleteDialog: (ids: number[]) => Promise<void>;
      isValidatingBulkDeleteInvoices: boolean;
    };
  ...
  if (invoicesSelectedRows.length > 0) {
    return (
      <div className="bigfin-ui">
        <div className="flex items-center gap-3 border-b border-border bg-surface px-4 py-2">
          <span className="text-sm font-medium text-text-secondary tabular-nums">
            {invoicesSelectedRows.length}
          </span>
          <Button
            variant="ghost"
            size="sm"
  ...
  return (
    <div className="bigfin-ui">
      <div className="flex flex-wrap items-center gap-2 border-b border-border bg-surface px-4 py-2">
        <Select
          value={invoicesViewSlug ?? ALL_VIEWS_VALUE}
        ...
        <Can I={SaleInvoiceAction.Create} a={AbilitySubject.Invoice}>
          <Button size="sm" className="gap-1.5" onClick={handleNewInvoiceBtnClick}>
            <Plus className="h-4 w-4" aria-hidden />
            {intl.get('new_invoice')}
          </Button>
        </Can>

        <InvoicesFilterPopover
          fields={invoicesFields}
          conditions={invoicesFilterConditions}
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="start">
            ...
            <DropdownMenuItem onClick={handleCustomizeBtnClick}>
              ...
              {intl.get('invoice.action.customize_templates')}
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
...
export const InvoicesToolbarV2 = compose(
  withInvoiceActions,
  withInvoices(
    ({
      invoicesSelectedRows,
      ...
      invoicesSelectedRows,
      invoicesFilterConditions: invoicesTableState.filterRoles ?? [],
      invoicesViewSlug: invoicesTableState.viewSlug ?? null,
    }),
  ),
  withDialogActions,
  withDrawerActions,
)(InvoicesToolbarV2Root) as ComponentType<Record<string, never>>;
