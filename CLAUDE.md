...
В репозитории лежат ЧЕТЫРЕ действующих мастер-ТЗ. Каждое следующее продолжает
...
- **ТЗ-3** (`docs/tz/MASTER-3.md`): блок Б5 «Управленческая глубина» по итогам
  ...
  деятельности для ОДДС плюс `pl_type` для ОПиУ). Закрыты по коду, ждут
  приёмки владельца (`docs/tz/ACCEPTANCE-B5.md`).
- **ТЗ-4** (`docs/tz/MASTER-4.md`): блок Б6 «Современный интерфейс» в духе Apple.
  Этапы 42–56. Главная мысль: Bigfin умеет больше конкурентов, но выглядит
  собранным из разных кусков — доводим до одного сдержанного языка (ясность,
  содержимое главнее хрома, одна главная кнопка, спокойное движение, телефон и
  тёмная тема первым классом) и ставим графики там, где они нужны, через общий
  набор `components/ui/charts/`. Меняется вид и удобство, не расчёты. Решения
  R1–R25 приняты по поручению владельца — не переигрывать. **Это текущая работа.**
...
| `docs/tz/MASTER-4.md` | ТЗ-4, этапы 42–56. Не менять без решения владельца. |
| `docs/tz/STATE.md` | **Где мы сейчас.** Источник правды: текущий этап, статусы всех 56 этапов, все четыре очереди, журнал. |
...
выполнения шага 0 (`git fetch` + чтение `docs/tz/STATE.md`). Какое из четырёх ТЗ
открывать — решает номер этапа: 1–15 в `MASTER.md`, 16–28 в `MASTER-2.md`,
29–41 в `MASTER-3.md`, 42–56 в `MASTER-4.md`.

**Очередей четыре, и порядок между ними жёсткий:** сначала незакрытое из Б1–Б3,
потом Б4, потом Б5, потом Б6. Пустая очередь сама по себе в стабилизацию не
отправляет. На 24.09.2026 очереди Б1–Б3, Б4 и Б5 закрыты по коду, а очередь Б6
полна: **следующий этап — 42 «Срочный ремонт интерфейса»**, раздел 9 файла
`MASTER-4.md`.

**Если работы по ТЗ нет** — пусты все четыре очереди либо все ждут владельца —
