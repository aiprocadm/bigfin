...
import {
  CustomersActionsMenuV2,
  type CustomerRow,
  type CustomerRowActions,
} from './CustomersActionsMenuV2';
...
/**
 * Колонки таблицы клиентов для нового DataTable (react-table v7 формат).
 */
export function useCustomersTableColumnsV2(actions: CustomerRowActions) {
  return useMemo(
    () => [
      {
        id: 'display_name',
      ...
      },
      {
        ...
        width: 48,
        Cell: ({ row }: { row: { original: CustomerRow } }) => (
          <CustomersActionsMenuV2 row={row.original} actions={actions} />
        ),
      },
    ],
    [actions],
