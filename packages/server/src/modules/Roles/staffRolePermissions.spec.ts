...
describe('засеянные права роли «Сотрудник»', () => {
  ...
  it('сотрудник может открыть приложенный файл, но не удалить его', () => {
    const ability = abilityFromSeed();

    // Прикрепить файл к документу можно и без права, а открыть — уже по праву.
    // Без просмотра сотрудник приложил бы договор к счёту и не смог бы его
    // потом открыть. Удаление — тяжелее повседневного, остаётся за владельцем.
  ...
  it('состав роли не изменился незаметно', () => {
    // Шесть предметов документов по четыре действия, просмотр вложений и три
    // справочника по три действия (без удаления). Если состав меняют
    // осознанно — меняется и это число.
    expect(staffRolePermissions()).toHaveLength(34);
  });
});
