...
const DEFAULT_FIELD_KEY = 'estimate_number';
 ...
 */
function EstimatesFilterPopover({
  fields,
  conditions,
  onFilterChange,
}: {
    ...
    <Popover open={open} onOpenChange={handleOpenChange}>
      ...
      <PopoverContent align="start" className="w-80">
        ...
        >
          ...
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="estimates-filter-comparator">
              {intl.get('toolbar.filter.condition')}
            </Label>
            <Select value={comparator} onValueChange={setComparator}>
              <SelectTrigger id="estimates-filter-comparator">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="estimates-filter-value">
              {intl.get('filter.value')}
            </Label>
            <Input
              id="estimates-filter-value"
              value={value}
...
interface EstimatesToolbarV2RootProps {
  // #withEstimates
  estimatesSelectedRows?: number[];
  estimatesFilterConditions?: FilterRole[];
  estimatesViewSlug?: string | null;

  // #withEstimatesActions
  setEstimatesTableState: (state: Record<string, unknown>) => void;
...
/**
 * Панель действий списка смет (shadcn, замена EstimatesActionsBar).
 */
function EstimatesToolbarV2Root({
  estimatesSelectedRows = [],
  estimatesFilterConditions = [],
  estimatesViewSlug,
  setEstimatesTableState,
  openDialog,
  openDrawer,
}: EstimatesToolbarV2RootProps) {
  const history = useHistory();

  // Контекст списка (легаси-провайдер без типов) — кастуем локально.
  const { estimatesViews = [], fields = [] } = (useEstimatesListContext() ??
    {}) as {
    estimatesViews?: ResourceView[];
    fields?: ResourceField[];
  };

  const { refresh } = useRefreshEstimates() as { refresh: () => void };
  ...
  const { openBulkDeleteDialog, isValidatingBulkDeleteEstimates } =
    useBulkDeleteEstimatesDialog() as {
      openBulkDeleteDialog: (ids: number[]) => Promise<void>;
      isValidatingBulkDeleteEstimates: boolean;
    };
  ...
  const handleNewEstimateBtnClick = () => {
  ...
  if (estimatesSelectedRows.length > 0) {
    return (
      <div className="bigfin-ui">
        <div className="flex items-center gap-3 border-b border-border bg-surface px-4 py-2">
          <span className="text-sm font-medium text-text-secondary tabular-nums">
            {estimatesSelectedRows.length}
          </span>
          <Button
            variant="ghost"
            size="sm"
  ...
  return (
    <div className="bigfin-ui">
      <div className="flex flex-wrap items-center gap-2 border-b border-border bg-surface px-4 py-2">
        <Select
          value={estimatesViewSlug ?? ALL_VIEWS_VALUE}
        ...
        <Can I={SaleEstimateAction.Create} a={AbilitySubject.Estimate}>
          <Button size="sm" className="gap-1.5" onClick={handleNewEstimateBtnClick}>
            <Plus className="h-4 w-4" aria-hidden />
            {intl.get('new_estimate')}
          </Button>
        </Can>

        <EstimatesFilterPopover
          fields={fields}
          conditions={estimatesFilterConditions}
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="start">
            ...
            <DropdownMenuItem onClick={handleCustomizeBtnClick}>
              ...
              {intl.get('estimate.action.customize_templates')}
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
...
export const EstimatesToolbarV2 = compose(
  withEstimatesActions,
  withEstimates(
    ({
      estimatesSelectedRows,
      ...
      estimatesSelectedRows,
      estimatesFilterConditions: estimatesTableState.filterRoles ?? [],
      estimatesViewSlug: estimatesTableState.viewSlug ?? null,
    }),
  ),
  withDialogActions,
  withDrawerActions,
)(EstimatesToolbarV2Root) as ComponentType<Record<string, never>>;
