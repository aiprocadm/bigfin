...
В репозитории лежит действующее мастер-ТЗ: переработка UX, консолидация по
юрлицам, закрытие пробелов относительно Финтабло/ПланФакта/Adesk. 15 этапов.

| Файл | Роль |
|---|---|
| `docs/tz/MASTER.md` | Само ТЗ. Не менять без решения владельца. |
| `docs/tz/STATE.md` | **Где мы сейчас.** Источник правды: текущий этап, статусы 15 этапов, очередь, журнал. |
...
по тз», «что дальше по тз», «продолжи работу по тз», `/tz`) — прочитать
`docs/tz/CONTINUE.md` и следовать ему буквально. Не импровизировать, не
начинать работу и не отчитываться до выполнения шага 0 (`git fetch` + чтение
`docs/tz/STATE.md`).

**Если работы по ТЗ нет** — все этапы приняты или очередь упёрлась в ожидание
владельца — переключиться в режим стабилизации: красный CI, падающие
