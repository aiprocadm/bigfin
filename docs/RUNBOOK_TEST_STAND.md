...
**Пароль стоит и на `/api` тоже — это осознанно.** Приложение кладёт токен в собственный заголовок `x-access-token`, а не в `Authorization`, поэтому с basic-auth он не конфликтует, и API остаётся под двойной защитой.
...
2. Есть новый → снимок `packages/webapp/dist` как страховка.
3. `git reset --hard` → зависимости **только если** сменился `pnpm-lock.yaml` → `pnpm build` → системные миграции.
