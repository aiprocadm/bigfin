...
В репозитории лежат ДВА действующих мастер-ТЗ. Второе продолжает первое, а не
отменяет его.
...
- **ТЗ-2** (`docs/tz/MASTER-2.md`): блок Б4 «Вход в продукт» по итогам живого
  ...
  отстаёт по входу в них. Это текущая работа.
...
| `docs/tz/STATE.md` | **Где мы сейчас.** Источник правды: текущий этап, статусы всех 28 этапов, обе очереди, журнал. |
...
выполнения шага 0 (`git fetch` + чтение `docs/tz/STATE.md`). Какое из двух ТЗ
открывать — решает номер этапа: 1–15 в `MASTER.md`, 16–28 в `MASTER-2.md`.

**Очередей две, и порядок между ними жёсткий:** сначала незакрытое из Б1–Б3,
потом Б4. Пустая очередь Б1–Б3 сама по себе в стабилизацию не отправляет.

**Если работы по ТЗ нет** — пусты обе очереди либо обе ждут владельца —
