...
/**
 * Скелетон загрузки деталей клиента: контур шапки + три карточки.
 * DrawerTitle с sr-only текстом сохраняет доступное имя диалога,
 * пока настоящее имя клиента ещё грузится.
 */
export function CustomerDetailsSkeletonV2() {
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <DrawerHeader className="shrink-0 gap-2 border-b border-border bg-surface px-4 py-4 pr-14 sm:px-6">
        <DrawerTitle className="sr-only">
          {intl.get('customer.drawer.loading')}
        </DrawerTitle>
