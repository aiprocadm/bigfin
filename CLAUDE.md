...
**Bigfin** — самостоятельный продукт российского управленческого учёта (аналог Fintablo, PlanFact), построенный на кодовой базе BigCapital. Целевая аудитория — российские предприниматели **без бухгалтерского образования**.
...
- **Bigfin** — единственное допустимое название во всём, что видит пользователь: UI, тексты, документация, переводы, email-шаблоны.
- **BigCapital** упоминается **только** в `LICENSE`, `DISCLAIMER` и подобных юридических файлах (требование upstream AGPL атрибуции).
- В переводах, заголовках, ошибках, страницах — **никогда** BigCapital, BigFin (camelCase), «БигКапитал». Всегда `Bigfin`.
