...
import { ACCOUNT_TYPE, Features, getAddMoneyInOptions } from '@/constants';
import {
  withDialogActions,
...
// ---------------------------------------------------------------------------
// Подтипы операции «Деньги пришли»: подпись и фильтр счёта-источника
// (паритет с легаси OwnerContribution / OtherIncome / TransferFromAccount).
// ---------------------------------------------------------------------------

const EMPTY_TYPES: string[] = [];

const MONEY_IN_SUBTYPES: Record<
  string,
...
> = {
  ...
  other_income: {
    creditLabelKey: 'cash_flow_transaction.other_income_account',
    creditFilterTypes: [ACCOUNT_TYPE.INCOME, ACCOUNT_TYPE.OTHER_INCOME],
  },
  transfer_from_account: {
    creditLabelKey: 'cash_flow_transaction.label_transfer_from_account',
    creditFilterTypes: [
      ACCOUNT_TYPE.CASH,
      ACCOUNT_TYPE.BANK,
 ...
 */
function MoneyInFormV2Root({
  accountId,
  accountType,
  onClose,
  openDialog,
}: MoneyInFormV2Props & WithDialogActionsProps) {
...
/**
 * Форма «Деньги пришли» (RHF + Zod + shadcn) по утверждённому концепту:
 * сумма крупно сверху, курс при мультивалюте, дата + текущий счёт,
 * тип операции + счёт-источник, детали (номер, документ, филиал), описание.
...
function MoneyInFormInner({
  ...
  const schema = useMemo(() => getMoneyInSchema(), []);
  const transactionTypeOptions = useMemo(
    () => getAddMoneyInOptions() as { name: string; value: string }[],
    [],
  );
  ...
  const form = useForm<MoneyInFormValues>({
  ...
  // Детали выбранного счёта — ради валюты (паритет MoneyInFieldsProvider).
  ...
  // Смена подтипа меняет допустимые счета-источники — сбрасываем выбор.
  ...
  // Подтверждение из диалога настройки нумерации (паритет MoneyInFormDialog).
  ...
  const onSubmit = async (values: MoneyInFormValues) => {
      ...
      <Form {...form}>
        ...
        >
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            {/* Текущий счёт (куда пришли деньги) */}
            <FormField
              control={form.control}
            ...
            {/* Счёт-источник — подпись и фильтр зависят от подтипа */}
            {subtype && (
              <FormField
                control={form.control}
...
export const MoneyInFormV2 = withDialogActions(MoneyInFormV2Root);
