...
/**
 * Схема формы склада (Zod, замена Yup).
 * Собирается фабрикой, чтобы intl-словарь был уже загружен к моменту вызова.
 */
export const getWarehouseFormSchema = () =>
  z.object({
    name: z
      .string()
      .trim()
      .min(1, intl.get('warehouse.dialog.validation.name_required')),
    code: z
      .string()
      .trim()
      .max(
...
export type WarehouseFormValues = z.infer<
  ReturnType<typeof getWarehouseFormSchema>
>;
