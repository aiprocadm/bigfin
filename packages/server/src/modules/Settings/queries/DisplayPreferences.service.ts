...
export const DISPLAY_PREFERENCE_DEFAULTS: Record<string, unknown> = {
  ...
  registryFilters: [],
  /**
   * Пропущенные шаги онбординга (FT-095 ТЗ-3). Пропуск — личное решение
   * человека, а не организации: владелец пропустил «пригласить финансиста»,
   * а приглашённому финансисту эта строка и так не показывается.
   */
  onboardingSkipped: [],
  /**
   * Человек хоть раз собрал отчёт (FT-095 ТЗ-3). В базе у отчёта нет
   * следа — он строится на лету и нигде не сохраняется, — поэтому отметку
   * ставит витрина, когда отчёт впервые пришёл с сервера.
   */
  onboardingReportBuilt: false,
  // Главная (FT-064 ТЗ-3): порядок блоков и скрытые блоки. Пустой порядок —
  // порядок по умолчанию; новый блок, которого нет в сохранённом порядке,
  // встаёт на своё обычное место, а не пропадает.
