...
/**
 * Итоги счёта (v2): подытог, скидка (с инпутом), корректировка (с инпутом),
 * налоги по ставкам, итого, оплачено, к оплате. Считается реактивно
 * из строк формы — той же математикой, что легаси-хуки useInvoiceTotal и др.
 */
export function InvoiceFormTotalsV2() {
  const form = useFormContext<InvoiceFormValues>();
  const { taxRates, invoice, isNewMode } = useInvoiceFormV2Context();
  ...
  const paidAmount = parseNumericInput(
    (invoice?.payment_amount as string | number | undefined) ?? 0,
  );
  ...
  const money = (amount: number) => formatInvoiceAmount(amount, currencyCode);
  ...
  return (
    <div className="ml-auto w-full max-w-sm divide-y divide-border">
      <TotalRow
        label={intl.get(
          isInclusiveTax
            ? 'invoice_form.label.subtotal_tax_inclusive'
            : 'invoice_form.label.subtotal',
        )}
      ...
      {/* ----------- Скидка на счёт ----------- */}
      <TotalRow
        label={intl.get('invoice_form.label.discount')}
      ...
      {/* ----------- Корректировка ----------- */}
      <TotalRow
        label={intl.get('invoice_form.label.adjustment')}
      ...
      {/* ----------- Итого ----------- */}
      <TotalRow
        label={`${intl.get('invoice_form.label.total')} (${currencyCode ?? ''})`}
      ...
      {!isNewMode && (
        <>
          <TotalRow
            label={intl.get('invoice_form.label.payment_amount')}
            value={money(totals.paidAmount)}
          />
          <TotalRow
            label={intl.get('invoice_form.label.due_amount')}
