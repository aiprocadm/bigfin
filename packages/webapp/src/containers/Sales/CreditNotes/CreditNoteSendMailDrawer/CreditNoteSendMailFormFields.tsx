...
import { useSendCreditNoteFormatArgsOptions } from './_hooks';
import { useSendMailItems } from '../../Estimates/SendMailViewDrawer/hooks';
import { useIsDarkMode } from '@/hooks/useDarkMode';

export function CreditNoteSendMailFormFields() {
  const argsOptions = useSendCreditNoteFormatArgsOptions();
    ...
    <Stack flex={1}>
      <Stack spacing={0} overflow="auto" flex="1" p={'30px'}>
        ...
        <Group>
          <FCheckbox
            name={'attachPdf'}
            label={intl.get('send_mail.label.attach_pdf')}
          />
        </Group>
      </Stack>
