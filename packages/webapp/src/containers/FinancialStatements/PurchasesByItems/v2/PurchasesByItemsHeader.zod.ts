...
/**
 * Схема формы настройки отчёта «Закупки по позициям» (замена Yup-схемы
 * из utils.tsx). Имена полей 1:1 с легаси-формой (данные и query не меняем).
 * Фабрика, чтобы intl.get вызывался после инициализации локали.
 */
export const getPurchasesByItemsHeaderSchema = () =>
  z
    .object({
      dateRange: z.string(),
      fromDate: z.date({
      ...
      // Из URL идентификаторы позиций могут прийти строками — не приводим.
...
export type PurchasesByItemsHeaderFormValues = z.infer<
  ReturnType<typeof getPurchasesByItemsHeaderSchema>
>;
