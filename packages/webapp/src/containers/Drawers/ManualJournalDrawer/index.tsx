...
const ManualJournalContentV2 = lazy(() =>
  import('./v2/ManualJournalContentV2').then((module) => ({
    default: module.ManualJournalContentV2,
  })),
);

interface ManualJournalDrawerProps {
  name: string;
  // #withDrawers
  isOpen?: boolean;
  payload?: { manualJournalId?: number | string };
}
...
/**
 * Drawer «Детали ручной проводки» (shadcn Drawer + v2-контент).
 * Механизм открытия прежний: redux openDrawer(DRAWERS.JOURNAL_DETAILS,
 * { manualJournalId }).
 */
function ManualJournalDrawerRoot({
  name,
  isOpen,
  payload,
  closeDrawer,
}: ManualJournalDrawerProps & WithDrawerActionsProps) {
  ...
  return (
    <Drawer open={Boolean(isOpen)} onOpenChange={handleOpenChange}>
      ...
      >
        ...
        >
          <ManualJournalContentV2 manualJournalId={payload?.manualJournalId} />
        </Suspense>
      </DrawerContent>
    </Drawer>
  );
...
export default compose(
  withDrawersLoose(),
  withDrawerActions,
)(ManualJournalDrawerRoot) as ComponentType<{ name: string }>;
