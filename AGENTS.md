# Bigfin — project memory for Codex
...
## Работа по мастер-ТЗ (точка входа)

В репозитории лежат ТРИ действующих мастер-ТЗ. Каждое следующее продолжает
предыдущее, а не отменяет его.

- **ТЗ-1** (`docs/tz/MASTER.md`): переработка UX, консолидация по юрлицам,
  закрытие пробелов относительно Финтабло/ПланФакта/Adesk. Этапы 1–15,
  блоки Б1–Б3. Закрыты по коду, ждут приёмки владельца.
- **ТЗ-2** (`docs/tz/MASTER-2.md`): блок Б4 «Вход в продукт» по итогам живого
  аудита ПланФакта. Этапы 16–28. Главная мысль: Bigfin не отстаёт по функциям —
  отстаёт по входу в них. Этапы влиты, остаток — приёмка владельца.
- **ТЗ-3** (`docs/tz/MASTER-3.md`): блок Б5 «Управленческая глубина» по итогам
  живого аудита Финтабло. Этапы 29–41. Главная мысль: после Б4 Bigfin не
  отстаёт ни по функциям, ни по входу — отстаёт по ГЛУБИНЕ управленческого
  смысла (план и факт в одной матрице ОДДС с сальдо и сцепкой периодов; ярусы
  прибыли МД → ВП1 → ВП2 → ОП → ЧП; статья как ДВОЙНОЙ классификатор — вид
  деятельности для ОДДС плюс `pl_type` для ОПиУ). **Это текущая работа.**

| Файл | Роль |
|---|---|
| `docs/tz/MASTER.md` | ТЗ-1, этапы 1–15. Не менять без решения владельца. |
| `docs/tz/MASTER-2.md` | ТЗ-2, этапы 16–28. Не менять без решения владельца. |
| `docs/tz/MASTER-3.md` | ТЗ-3, этапы 29–41. Не менять без решения владельца. |
| `docs/tz/STATE.md` | **Где мы сейчас.** Источник правды: текущий этап, статусы всех 41 этапа, все три очереди, журнал. |
| `docs/tz/CONTINUE.md` | **Алгоритм продолжения работы.** Читать и выполнять буквально. |
| `.claude/commands/tz.md` | Слэш-команда `/tz`. |
| `.claude/skills/tz-continue/` | Локальный скилл-указатель. Не в git (`.claude/skills/` в `.gitignore`) — механизм работает и без него. |

**Когда владелец пишет «продолжай по ТЗ»** (или любую близкую фразу: «работай
по тз», «что дальше по тз», «продолжи работу по тз», «продолжаем по тз»,
«дальше по тз», `/tz`) — прочитать `docs/tz/CONTINUE.md` и следовать ему
буквально. Не импровизировать, не начинать работу и не отчитываться до
выполнения шага 0 (`git fetch` + чтение `docs/tz/STATE.md`). Какое из трёх ТЗ
открывать — решает номер этапа: 1–15 в `MASTER.md`, 16–28 в `MASTER-2.md`,
29–41 в `MASTER-3.md`.

**Очередей три, и порядок между ними жёсткий:** сначала незакрытое из Б1–Б3,
потом Б4, потом Б5. Пустая очередь Б1–Б3 сама по себе в стабилизацию не
отправляет. На 21.09.2026 очереди Б1–Б3 и Б4 закрыты по коду, а очередь Б5
полна: **следующий этап — 29**, раздел 25 файла `MASTER-3.md`.

**Если работы по ТЗ нет** — пусты все три очереди либо все ждут владельца —
переключиться в режим стабилизации: красный CI, падающие
тесты, ошибки типов, непарные переводы, долг по `@ts-nocheck`, сторожевые
спеки, дыры в e2e. Порядок источников — в разделе 5 файла `docs/tz/STATE.md`.

**Обязанность каждой сессии:** в конце шага обновить `docs/tz/STATE.md` и
закоммитить вместе с работой. Не обновили — следующая сессия начнёт не с той
точки, и весь механизм перестаёт работать.
...
В репозитории НЕ хранятся скиллы/хуки Codex (они в `~/.Codex/` пользователя). Однако в репо есть **установщики**, которые их разворачивают:

- `apply-Codex-automation.js` (в parent-каталоге) — устанавливает:
...
- MCP сервер `context7` — рекомендуется добавить: `Codex mcp add --transport http context7 https://mcp.context7.com/mcp` (живая документация по NestJS, Blueprint, Radix, Stripe, BullMQ и пр.).
