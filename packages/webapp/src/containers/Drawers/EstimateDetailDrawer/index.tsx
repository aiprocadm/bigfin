...
const EstimateDetailContentV2 = lazy(() =>
  import('./v2/EstimateDetailContentV2').then((module) => ({
    default: module.EstimateDetailContentV2,
  })),
);
...
/**
 * Drawer «Детали сметы» (shadcn Drawer + карточки на v2-контенте).
 * Механизм открытия прежний: redux openDrawer(DRAWERS.ESTIMATE_DETAILS,
 * { estimateId }).
 */
function EstimateDetailDrawerRoot({
  name,
  isOpen,
  payload,
  closeDrawer,
}: EstimateDetailDrawerProps & WithDrawerActionsProps) {
  ...
  return (
    <Drawer open={Boolean(isOpen)} onOpenChange={handleOpenChange}>
      ...
      >
        ...
        >
          <EstimateDetailContentV2 estimateId={payload?.estimateId} />
        </Suspense>
      </DrawerContent>
    </Drawer>
  );
...
export default compose(
  withDrawersLoose(),
  withDrawerActions,
)(EstimateDetailDrawerRoot) as ComponentType<{ name: string }>;
