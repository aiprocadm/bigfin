...
// Два РАЗНЫХ прихода с одинаковыми номером, датой и суммой (две одинаковые
// комиссии за день) — базовый ключ у них совпадает, разводится суффиксом.
const FIXTURE_WITH_COLLISION = [
  '1CClientBankExchange',
  'ВерсияФормата=1.03',
  'Кодировка=Windows',
  'РасчСчет=40702810400000012345',
  ...
  'ПолучательРасчСчет=40702810400000012345',
  'ПолучательИНН=7707654321',
  'НазначениеПлатежа=Комиссия банка',
  'КонецДокумента',
  'СекцияДокумент=Платежное поручение',
  'Номер=300',
  'Дата=15.06.2026',
...
describe('Import1CStatementService', () => {
  ...
  // -------------------------------------------------------------------------
  // (e) две разные операции с одинаковым базовым ключом не схлопываются (И1 срез 2)
  // -------------------------------------------------------------------------
  it('(e) два одинаковых по номеру/дате/сумме прихода → обе загружены с разными ключами', async () => {
    const buf = Buffer.from('﻿' + FIXTURE_WITH_COLLISION, 'utf8');
    const result = await service.import(42, '40702810400000012345', 'RUB', buf);
    ...
    const ids = (createService.create as jest.Mock).mock.calls.map(
      (c) => c[0].externalId,
    );
    expect(ids).toEqual(['1c:300:2026-06-15:250.00', '1c:300:2026-06-15:250.00#2']);
  });
