...
/**
 * Подтверждение удаления клиента (shadcn ConfirmDialog).
 * Механизм прежний: redux openAlert('customer-delete', { contactId }).
 */
function CustomerDeleteAlertRoot({
  name,
  isOpen,
  payload,
  closeAlert,
  ...
  // Подтверждение: удаляем клиента, показываем тост, закрываем drawer.
  ...
  }, [deleteCustomerMutate, contactId, closeDrawer, closeAlert, name]);

  return (
    <ConfirmDialog
      open={Boolean(isOpen)}
      title={intl.get('delete_customer')}
...
export default compose(
  ...
  withAlertActions,
  withDrawerActions,
)(CustomerDeleteAlertRoot) as ComponentType<CustomerDeleteAlertProps>;
