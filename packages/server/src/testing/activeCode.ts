...
/**
 * Карта v45 (витрина) и v46 (сервер). Действующий код файла — без
 * комментариев.
 *
 * ПАРНАЯ КОПИЯ файла `packages/webapp/src/testing/activeCode.ts`. Копия, а
 * не общий пакет: это утилита проверок, а у пакетов разные тестовые
 * движки, и тащить её в продуктовый `@bigfin/utils` значит везти тестовый
 * код в сборку продукта. За тем, чтобы копии не разъехались, следит
 * отдельный сторож `activeCodeCopiesMatch.spec.ts`.
 *
