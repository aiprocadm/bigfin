...
import type { BillPaymentTransaction } from './types';

interface BillPaymentTransactionsTabV2Props {
  billId: number;
}
...
// useBillPaymentTransactions — легаси react-query хук без типов.
interface UseBillPaymentTransactionsResult {
  data: BillPaymentTransaction[];
  isLoading: boolean;
  isFetching: boolean;
}

type PaymentRow = BillPaymentTransaction & { __rowId: string };
...
/**
 * Вкладка «Операции по платежам»: платежи по счёту поставщика
 * с действиями «редактировать/удалить» (как в легаси ContextMenu).
 */
function BillPaymentTransactionsTabV2Root({
  billId,
  openAlert,
  closeDrawer,
}: BillPaymentTransactionsTabV2Props &
  WithAlertActionsProps &
  WithDrawerActionsProps) {
  const history = useHistory();

  const { data, isLoading, isFetching } = useBillPaymentTransactions(billId, {
    enabled: !!billId,
  }) as UseBillPaymentTransactionsResult;
  ...
  const rows = useMemo<PaymentRow[]>(
    () =>
      (data ?? []).map((transaction, index) => ({
        ...transaction,
        __rowId:
          transaction.bill_payment_id != null
            ? String(transaction.bill_payment_id)
            : `row-${index}`,
      })),
    [data],
  );
  ...
  const columns = usePaymentColumnsV2({
    onEdit: (row) => {
      history.push(`/payments-made/${row.bill_payment_id}/edit`);
      closeDrawer(DRAWERS.BILL_DETAILS);
    },
    onDelete: (row) => {
      openAlert('payment-made-delete', {
        paymentMadeId: row.bill_payment_id,
      });
    },
  });
...
export const BillPaymentTransactionsTabV2 = compose(
  withAlertActions,
  withDrawerActions,
)(BillPaymentTransactionsTabV2Root) as ComponentType<BillPaymentTransactionsTabV2Props>;
