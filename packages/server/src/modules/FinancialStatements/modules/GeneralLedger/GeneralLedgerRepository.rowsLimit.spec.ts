// © 2026 Bigfin
import { GeneralLedgerRepository } from './GeneralLedgerRepository';
import { REPORT_ROWS_LIMIT } from '../../common/reportRowsLimit';

/**
 * М3 срез 3 (карта v15): Главная книга грузила в память ВСЕ проводки периода
 * без всякого потолка. Отчёт за большой период отвечал минуту или падал.
 *
 * Главное в этом тесте — что отказ происходит ДО выборки строк: иначе
 * предохранитель бесполезен, память уже съедена.
 */
const buildRepository = (rowsCount: number, rows: any[] = []) => {
  const repository = new GeneralLedgerRepository() as any;

  repository.filter = {
    fromDate: '2026-01-01',
    toDate: '2026-12-31',
    branchesIds: [],
  };
  const calls = { counting: 0, selecting: 0 };

  repository.accountTransactionModel = () => ({
    query: () => ({
      onBuild: (callback: (query: any) => void) => {
        const builder: any = {
          modify: jest.fn(),
          orderBy: jest.fn(),
          whereIn: jest.fn(),
          withGraphFetched: jest.fn(),
          count: jest.fn(),
        };
        callback(builder);

        // Подсчёт узнаём по вызову count() — так тест не зависит от того,
        // в каком порядке репозиторий строит запросы.
        if (builder.count.mock.calls.length > 0) {
          calls.counting += 1;
          return Promise.resolve([{ rowsCount }]);
        }
        calls.selecting += 1;
        return Promise.resolve(rows);
      },
    }),
  });
  return { repository, calls };
};

describe('потолок строк Главной книги', () => {
  it('обычный отчёт собирается как раньше', async () => {
    const rows = [
      {
        id: 1,
        accountId: 100,
        credit: 0,
        debit: 500,
        date: '2026-01-12',
        // Книга читает у проводки её счёт — без него сборка падает.
        account: { accountNormal: 'debit' },
      },
    ];
    const { repository, calls } = buildRepository(1, rows);

    await repository.initTransactions();

    expect(repository.transactions).toHaveLength(1);
    expect(calls.selecting).toBe(1);
  });

  it('строк больше потолка — отказ, и строки НЕ загружаются', async () => {
    const { repository, calls } = buildRepository(REPORT_ROWS_LIMIT + 1);
    let error: any;

    try {
      await repository.initTransactions();
    } catch (caught) {
      error = caught;
    }
    expect(error?.errorType).toBe('REPORT_ROWS_LIMIT_EXCEEDED');
    expect(calls.selecting).toBe(0);
  });

  it('в отказе видно, сколько строк вышло', async () => {
    const { repository } = buildRepository(REPORT_ROWS_LIMIT + 7);
    let error: any;

    try {
      await repository.initTransactions();
    } catch (caught) {
      error = caught;
    }
    expect(error?.payload?.rowsCount).toBe(REPORT_ROWS_LIMIT + 7);
    expect(error?.payload?.limit).toBe(REPORT_ROWS_LIMIT);
  });

  it('подсчёт делается ровно один раз', async () => {
    const { repository, calls } = buildRepository(5, []);

    await repository.initTransactions();

    expect(calls.counting).toBe(1);
  });
});
