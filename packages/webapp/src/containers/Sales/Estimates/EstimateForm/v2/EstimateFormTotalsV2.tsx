...
/**
 * Итоги сметы (v2): подытог, скидка (с инпутом), корректировка (с инпутом),
 * итого. Налогов у сметы нет (легаси: enableTaxRates=false). Считается
 * реактивно из строк формы — той же математикой, что легаси useEstimateTotal.
 */
export function EstimateFormTotalsV2() {
  ...
  const money = (amount: number) => formatEstimateAmount(amount, currencyCode);

  return (
    <div className="ml-auto w-full max-w-sm divide-y divide-border">
      <TotalRow
        label={intl.get('estimate_form.label.subtotal')}
      ...
      {/* ----------- Итого ----------- */}
      <TotalRow
        label={`${intl.get('estimate_form.label.total')} (${currencyCode ?? ''})`}
