...
const HIDDEN_ON_PURPOSE = [
  ...
  // Переключатель «кассовый метод / метод начисления» в шапке ОПиУ.
  // Это не раздел, а часть работающего отчёта: без флага accrual_pnl
  // движок второй метод не считает, и объяснение на месте двух кнопок
  // предлагало бы включить модуль ради переключателя, который человек
  // и не искал. Сам отчёт при этом открыт и показывает цифры.
  'containers/FinancialStatements/ProfitLossSheet/v2/ProfitLossBasisSwitch.tsx',
];
